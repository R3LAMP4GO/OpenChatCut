...
try {
  ...
  // The incoming visual starts 250 ms after its incoming audio, modelling a real J-cut
  // that uses source preroll rather than incorrectly offsetting lip-synced source audio.
  run('ffmpeg', [
  ...
  run('ffmpeg', [
    '-y', '-f', 'lavfi', '-i', 'color=c=red:s=320x180:r=30:d=3.5',
    '-f', 'lavfi', '-i', 'sine=frequency=660:sample_rate=48000:duration=3.5',
  ...
  run('ffmpeg', [
    ...
    '[0:v]trim=duration=3,setpts=PTS-STARTPTS[v0];'
      + '[1:v]trim=start=0.25:end=3.25,setpts=PTS-STARTPTS[v1];'
      + '[v0][v1]concat=n=2:v=1:a=0[v];'
      + '[0:a]atrim=duration=3,asetpts=PTS-STARTPTS[a0];'
      + '[1:a]atrim=duration=3.25,asetpts=PTS-STARTPTS,adelay=2750:all=1[a1];'
  ...
  // Probe 2.80–2.90 seconds. 660 Hz energy proves incoming audio has begun while
  // the outgoing visual is still present. RMS guards against an empty/silent probe.
  run('ffmpeg', ['-y', '-ss', '2.80', '-t', '0.10', '-i', result, '-map', '0:a:0', '-f', 's16le', '-ac', '1', '-ar', '48000', audioProbe]);
  ...
  console.log('j-cut-editor.verify: ok (skill safety rules + 250 ms audio-led cut simulation)');
} finally {
