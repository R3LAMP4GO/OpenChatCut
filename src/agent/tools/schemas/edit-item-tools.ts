    ...
    input_schema: {
      ...
      properties: {
        adds: {
          type: 'array',
          description:
            'effect: {type,targetItemId,assetId,propertyOverrides?}. transition: {type,assetId,incomingItemId,outgoingItemId?,durationInFrames?}. motion-graphic: {type,assetId:library:motion-graphic:*,track?,fromFrame|startFrame?}. audio SFX: {type:"audio",assetId:library:sound:*,fromFrame?}. POOL media B-roll (video/image/gif/svg/audio): {type,assetId,track|trackId?,fromFrame|startFrame?,durationInFrames?} — no name/props on pool adds (then update_item_props). Use type:"audio" with a video pool asset to create a raw audio-only timeline copy for J/L cuts; mute the overlapping video audio to prevent doubling. Source-window add (video/audio/gif): {type,assetId,sourceStartMs?,sourceEndMs?,sourceStartSeconds?,sourceEndSeconds?,track?,fromFrame?} — pass search_media hit *Ms fields through unchanged; use either milliseconds or seconds for each bound, never both. The tool converts to frames; do NOT combine source windows with durationInFrames/srcInFrame, and do not multiply by fps yourself. Authored text: {type:"text",text,name?,track?,fromFrame?,durationInFrames?,fontSize?,color?,fontWeight?,align?}. Authored solid: {type:"solid",color?,name?,track?,fromFrame?,durationInFrames?}.',
          items: { type: 'object' },
        },
        updates: {
          type: 'array',
